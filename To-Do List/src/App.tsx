...
function App() {
  ...
  const [inputEdit,setİnputEdit]=useState<string>("");
  const[selectedİd,setSelectedİd]=useState<string>("");
  ...
  const openEditModal=(id:string, content:string)=>{
    setOpen(true)
    setİnputEdit(content)
    setSelectedİd(id)
  }

  const saveEditedİtem= ()=>{
    const selectedİtem=items.find((item)=>item.id === selectedİd)
    if(selectedİtem){
      selectedİtem.content=inputEdit
      setItems([...items])
    }
    setOpen(false)
  }
        ...
        open&&(
          <ModalBackground onClick={closeEditedModal}>
            <ModalContainer onClick={(e)=> e.stopPropagation()}>
              ...
              type='text'
              value={inputEdit}
              onChange={(e)=>setİnputEdit(e.target.value)}
              />
              <Button onClick={saveEditedİtem}>Ok </Button>
            </ModalContainer>
          </ModalBackground>
        )
      }
